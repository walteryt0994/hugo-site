...
import { Mail, MapPin, Github, Linkedin, Camera, Database, Cpu, Brain, Rocket, Award, GraduationCap, Phone } from "lucide-react";
...
const DATA = {
  ...
  links: {
    github: "#",
    linkedin: "#",
    resume: "/Resume.pdf", // ← 部署后应使用 public 下的路径
  },
  aboutEN:
...
export default function PersonalSite() {
  ...
  const headline = useMemo(() => (lang === "EN" ? DATA.headlineEN : DATA.headlineZH), [lang]);
  const about = useMemo(() => (lang === "EN" ? DATA.aboutEN : DATA.aboutZH), [lang]);
    ...
    <main className="min-h-screen bg-gradient-to-b from-white to-slate-50">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3 text-sm text-slate-700">
              ...
              <a className="inline-flex items-center gap-1 hover:underline" href={`mailto:${DATA.email}`}><Mail size={16} />{DATA.email}</a>
              <span className="inline-flex items-center gap-1"><Phone size={16} />{DATA.phone}</span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="secondary" className="rounded-2xl" onClick={() => setLang(lang === "EN" ? "ZH" : "EN")}>
              {lang === "EN" ? "中文" : "EN"}
            </Button>
      ...
      <div className="max-w-5xl mx-auto px-6 mb-8">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <Badge><GraduationCap className="inline mr-1" size={16}/> NEU MSIS</Badge>
          <Badge><Cpu className="inline mr-1" size={16}/> Robotics / ROS</Badge>
          <Badge><Database className="inline mr-1" size={16}/> Python / SQL</Badge>
          <Badge><Brain className="inline mr-1" size={16}/> ML: TF/Keras</Badge>
        </div>
      </div>

      {/* About */}
      <Section title={lang === "EN" ? "About" : "关于我"} icon={<Camera size={18} className="text-slate-700" />}>
        <p className="text-slate-700 leading-7">{about}</p>
      </Section>

      {/* Experience */}
      <Section title={lang === "EN" ? "Experience" : "经历"} icon={<Rocket size={18} className="text-slate-700" />}>
      ...
      {/* Projects */}
      <Section title={lang === "EN" ? "Projects" : "项目"} icon={<Award size={18} className="text-slate-700" />}>
      ...
      {/* Skills */}
      <Section title={lang === "EN" ? "Skills" : "技能"} icon={<Cpu size={18} className="text-slate-700" />}>
          ...
          {DATA.sections.skills.map((s, i) => (
            <div key={i}>
              ...
              <div className="flex flex-wrap gap-2">
                {s.items.map((it, j) => (
                  <span key={j} className="px-2 py-1 rounded-xl bg-slate-100 text-slate-700 text-sm">{it}</span>
                ))}
              </div>
            </div>
          ))}
      ...
      {/* Contact */}
      <Section title={lang === "EN" ? "Contact" : "联系"} icon={<Mail size={18} className="text-slate-700" />}>
        <div className="flex flex-wrap items-center gap-3 text-slate-700">
          <a className="inline-flex items-center gap-2 hover:underline" href={`mailto:${DATA.email}`}><Mail size={16} /> {DATA.email}</a>
          <span className="inline-flex items-center gap-2"><Phone size={16} /> {DATA.phone}</span>
          <a className="inline-flex items-center gap-2 hover:underline" href={DATA.links.github} target="_blank" rel="noreferrer"><Github size={16} /> GitHub</a>
          <a className="inline-flex items-center gap-2 hover:underline" href={DATA.links.linkedin} target="_blank" rel="noreferrer"><Linkedin size={16} /> LinkedIn</a>
        </div>
      </Section>
